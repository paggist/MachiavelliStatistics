...
def solve(cards, optional_cards=()):
    ...
    print("card_mat",card_mat.T.shape)
    print("card_min",card_min.shape)
    print("card_max",card_max.shape)
    print("group_max",group_max.shape)
    constraints = [
...
def create_identical_pairs_hand(eqlength,deck):
    # Count occurrences of each card
    card_counts = Counter(deck)

    # Find three cards with at least two identical copies
    chosen_cards = [card for card, count in card_counts.items() if count >= 2][:eqlength]

    if len(chosen_cards) < eqlength:
        raise ValueError("Not enough identical cards in the deck to create three pairs.")

    # Extract two identical cards for each chosen card
    starting_hand = []
    for card in chosen_cards:
        starting_hand.extend([card] * 2)  # Add the pair to the hand
        deck.remove(card)  # Remove one copy from the deck
        deck.remove(card)  # Remove the second copy

    return starting_hand, deck

def create_suit_hand(eqlength,deck, suit):
    # Filter the deck for the chosen suit
    suit_cards = [card for card in deck if card.endswith(suit)]

    if len(suit_cards) < eqlength:
        raise ValueError(f"Not enough cards in the suit '{suit}' to create a hand.")

    # Select three cards from the suit
    hand = random.sample(suit_cards, eqlength)
...
def gameEqualCards(nstartcards=5,ndecks=2,nplayers=2,neqcards=1):
    ...
    for i in range(nplayers):
        if i==0:
            hand,DECK=create_identical_pairs_hand(neqcards, DECK)
           
            if len(hand)<nstartcards:
                lenhand=len(hand)
                
                hand.extend(DECK[:nstartcards-lenhand])
                
                DECK=DECK[nstartcards-lenhand:]
            HANDS.append(hand)
...
def gameEqualSuit(nstartcards=5,ndecks=2,nplayers=2,neqsuit=2):
    global data_to_write_to_file
    # nstartcards=5
    # ndecks=2
    # nplayers=3
    ...
    for i in range(nplayers):
        if i==0:
            hand,DECK=create_suit_hand(neqsuit, DECK,'s')
...
#     for equalcards in [1,2,3,4,5,6]:
#         data_to_write_to_file=[]
#         filename="multiEqualCards/machiagame_players"+str(players)+"_equalcards_"+str(equalcards)+"_"+str(time.time())+".csv"
...
#             if len(data_to_write_to_file)>1000 or i==recursions-1:
#                 write_csv(filename, data_to_write_to_file,append=True)
#                 data_to_write_to_file=[]
#             g,rounds=gameEqualCards(nstartcards,ndecks,nplayers,equalcards)
#             if g!=-1 and g!=-99:
#                 result[g]+=1
#             elif g==-99:
#                 result[nplayers+1]+=1
...
#         write_csv("multiEqualCards/machiagamem3solve_results.csv",[[result,resultrounds,elapsed_time,time.time()]],append=True)

for players in [3,4,5,6]:
    for equalcolour in [4,6,12]:
        data_to_write_to_file=[]
        # filename="multiEqualColour/machiagame_players"+str(players)+"_equalcolour_"+str(equalcolour)+"_"+str(time.time())+".csv"
        ...
        for i in range(recursions):
            ...
            if len(data_to_write_to_file)>1000 or i==recursions-1:
                write_csv(filename, data_to_write_to_file,append=True)
                data_to_write_to_file=[]
            g,rounds=gameEqualSuit(nstartcards,ndecks,nplayers,equalcolour)
        ...
        # write_csv("multiEqualColour/machiagamem3solve_results.csv",[[result,resultrounds,elapsed_time,time.time()]],append=True)
